...
df_log = df_log.sort_values(by=['user', 'time'])

# Step 2: Generate transitions
df_log['next_section'] = df_log.groupby('user')['name'].shift(-1)
transitions = df_log.dropna(subset=['next_section']).groupby(['name', 'next_section']).size().reset_index(name='count')

# Step 3: Calculate total outgoing transitions for each source section
outgoing_totals = transitions.groupby('name')['count'].sum().to_dict()

# Step 4: Create a transition matrix
all_sections = sorted(set(df_log['name']).union(set(df_log['next_section'].dropna())))
transition_matrix = pd.DataFrame(0, index=all_sections, columns=all_sections)

for _, row in transitions.iterrows():
    if outgoing_totals[row['name']] > 0:  # Normalize using total outgoing transitions
        transition_matrix.at[row['next_section'], row['name']] = (row['count'] / outgoing_totals[row['name']]) * 100
...
plt.xlabel('From Element', fontsize=12)
plt.ylabel('To Element', fontsize=12)
plt.xticks(rotation=90, fontsize=10, ha='center')
plt.yticks(rotation=0, fontsize=10, va='center')
plt.tight_layout()
plt.savefig(df_log["course"].iloc[0]+".png")
